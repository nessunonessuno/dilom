...
export default function SendTransaction() {
 ...
 console.log(write)
  ...
  return (
    <>
      {isConnected ? chain.id != chains[0].id ? <Button className="minter"><Image src={wrong}/></Button> : <Button className="minter" isDisabled={!write || isLoading} onClick={() => write()}>{isLoading ? 'Minting...' : 'Mint Now'}</Button> :<Button className="minter" isDisabled={!write || isLoading} onClick={() => write()}>{isLoading ? 'Minting...' : 'Mint Now'}</Button>}
